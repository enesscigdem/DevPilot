...
public class EditApplicabilityAndRepairTests : IDisposable
{
    ...
    public async Task EditRepair_RequestContainsExactCurrentTargetSourceAndFailedSearchEvidence()
    {
        ...
        repairPrompt.Should().Contain("Edit Strategy: surgical patch.");
        repairPrompt.Should().Contain("smallest verbatim search anchors");
        repairPrompt.Should().Contain("Output ONLY the corrected surgical edit JSON for 'Service.cs'.");
        repairPrompt.Should().NotContain("hash-guarded small-file replacement");
