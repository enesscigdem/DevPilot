...
import { getBrainStatus, indexBrain, askBrain } from "@/api"
...
export function ProjectBrain() {
  ...
  // When active workspace changes: clear conversation, citations, and reload status
  useEffect(() => {
  ...
  const handleAsk = async (questionToAsk?: string) => {
    ...
    try {
      const res = await askBrain(activeWorkspaceId, q)
      if (res.success) {
